...
Minimal release package for the paper: the production simulator, its configuration,
and the plotted data behind every figure panel.

## Contents

- `main.cpp` — the production N-swimmer / N-passive Brownian-dynamics simulation
...
- `config.yml` — the production configuration (HRHS kick parameters shown;
  `kick_rate` / `kick_torque` are the swept pair).
- `runscript.sh` — builds and runs the simulator: `./runscript.sh [multiplier]`
  (default 160, the production time step `dt = 0.1/160 = 6.25e-4 s`).
  Requires `g++` (C++17) and yaml-cpp (`brew install yaml-cpp` /
  `apt install libyaml-cpp-dev`).
- `figures/` — one `figure_<n>_<panel>.csv` + `figure_<n>_<panel>.py` pair per
  data-bearing panel of the manuscript (main Figs. 1–4) and the SM (Figs. S1–S4).
  Each CSV holds the final plotted values (x, y per series — no further
...
| `figure_2_g` | Polar rose of exit angle across five kick strengths |
...
All CSVs were extracted from the analysis caches of the full research pipeline
(no re-simulation): Fig. 1 from KDE/histogram caches of two 80,000 s corner
trajectories; Fig. 2 from cached 2-swimmer fixed-initial-condition scattering
episodes; Figs. 3 and 4(a) from the 4,096-run Saltelli sweep; Fig. 4(b–e) and
S4 from the signed Markov-transition-matrix fields of the four corner
trajectories; S1 from a streaming pass over the HRHS trajectory plus the
time-step-scan summaries. Raw trajectories (~4 GB each) are not included.
